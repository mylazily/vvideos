...
### 4.4 数据库分片约束（强制）
- **所有视频按 vod_id 数字编号分片到 10 个数据库（DB_0 - DB_9）**
- 分片规则：提取ID中所有数字，拼接后取模10
  - 例：`mpg9fr6gpafsu` → 提取数字 `96` → 96 % 10 = 6 → **DB_6**
  - 例：`mpg9fqw7zqoz5` → 提取数字 `975` → 975 % 10 = 5 → **DB_5**
- 无数字的 ID 回退到 FNV-1a 哈希分片
- ❌ **禁止修改分片规则**
