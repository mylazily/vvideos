<script lang="ts">
  ...
  onMount(() => {
    if (searchQuery) {
      keyword = searchQuery;
      doSearch(pageParam || 1);
    }
  });

  async function doSearch(pg: number) {
    ...
    try {
      const res = await fetch(
        '/api/search?q=' + encodeURIComponent(keyword.trim()) + '&page=' + pg + '&limit=24',
  ...
  function handleSearch() {
    if (keyword.trim()) {
      goto('/search/' + encodeURIComponent(keyword.trim()) + '/1');
    }
  }

  function handlePageChange(pg: number) {
    goto('/search/' + encodeURIComponent(keyword.trim()) + '/' + pg);
...
<svelte:head>
  ...
  <link rel="canonical" href="https://evideos.pages.dev/search/{encodeURIComponent(keyword)}" />
</svelte:head>
