...
<div class="min-h-screen bg-gray-50">
  ...
  <header class="sticky top-0 bg-white border-b border-gray-100 px-3 py-2 z-50">
    <form action="/search" method="get" class="flex items-center h-9 px-3 bg-gray-100 rounded-lg">
      <input name="q" type="text" placeholder="搜索电影、电视剧、动漫..."
        class="flex-1 bg-transparent text-sm outline-none" autocomplete="off" />
      <button type="submit" class="text-pink-500 text-sm">搜索</button>
    </form>
  </header>

  <!-- 视频列表：手机端一行两个，始终 grid-cols-2 -->
  <main class="pb-14">
    {#if data.videos && data.videos.length > 0}
      <div id="vg" class="grid grid-cols-2 gap-1.5 px-1.5 pt-1.5">
        {#each data.videos as video, i (video.vod_id)}
          <a href="/v/{video.vod_id}" class="group block">
            <div class="relative aspect-video bg-gray-200 rounded overflow-hidden">
              <img
                src={video.cover}
                alt={video.title}
              ...
              {#if video.vod_remarks}
                <div class="absolute bottom-0.5 right-0.5 px-1 py-px bg-black/70 text-white text-[10px] rounded">
                  {video.vod_remarks}
                </div>
              {/if}
            </div>
            <h3 class="mt-1 text-xs text-gray-800 line-clamp-2 leading-tight">
              {video.title}
            </h3>
          </a>
        {/each}
  ...
  <nav class="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 z-50">
    <div class="flex items-center justify-around h-11">
      <a href="/" class="flex-1 h-full flex items-center justify-center text-xs text-pink-500 font-medium" aria-current="page">首页</a>
      <a href="/discover" class="flex-1 h-full flex items-center justify-center text-xs text-gray-500">发现</a>
      <a href="/category/全部/1" class="flex-1 h-full flex items-center justify-center text-xs text-gray-500">分类</a>
      <a href="/profile" class="flex-1 h-full flex items-center justify-center text-xs text-gray-500">我的</a>
    </div>
  </nav>
</div>
